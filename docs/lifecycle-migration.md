...
| Lua discovery accepted multiple Lua versions and LuaJIT. | Lua tooling is Lua 5.5 only, with `lua-env`, `lua-rock`, `lua-test`, and release Lua artifact checks. | Existing Lua facade behavior is preserved. | Lua target, rockspec, source-rock, and release artifact tests. | Pending |
| Compose-backed e2e had project-specific command names only. | Standard `test-e2e`, `dev-up`, `dev-down`, `dev-reset`, `dev-ps`, and `dev-logs` surfaces wrap the existing deterministic local rig. | Existing curl/OIDC/M2M e2e tests remain available as focused targets. | Make help and e2e wrapper tests. | Pending |
